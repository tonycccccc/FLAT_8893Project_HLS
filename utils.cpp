...
void Load_Bias_from_DRAM(data_t bias_buffer[BATCH_B][NUM_HEAD_N][QUERY_LENGTH_F][HEAD_DIM_H], data_t bias[1][16][64][64])
{
    for (int b = 0; b < BATCH_B; ++b)
    {
		#pragma HLS pipeline off
    ...
    for (int b = 0; b < BATCH_B; ++b)
    {
        ...
        for (int t = 0; t < 64; ++t)
        {
            ...
            for (int n = 0; n < 16; ++n)
            {
                ...
                {
					#pragma HLS pipeline off
                    logit_out[idx*BATCH_B + b][t][n][h] = logit_out_buffer[b][t][n][h];

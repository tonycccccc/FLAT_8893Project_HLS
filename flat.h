...
void Load_Key_from_DRAM(data_t key_buffer[BATCH_B][KEY_LENGTH_T][NUM_HEAD_N][HEAD_DIM_H], data_t key[64][64][16][64]);
void Load_Value_from_DRAM(data_t value_buffer[BATCH_B][KEY_LENGTH_T][NUM_HEAD_N][HEAD_DIM_H], data_t value[64][64][16][64]);
void Load_Bias_from_DRAM(data_t bias_buffer[BATCH_B][NUM_HEAD_N][QUERY_LENGTH_F][KEY_LENGTH_T], data_t bias[64][64][16][64]);

void Fused_Logit_Operator(data_t query_buffer[64][64][16][64], data_t key_buffer[64][64][16][64], 
                data_t out_buffer[64][16][64][64]);
void Softmax(data_t logit_out[64][16][64][64], data_t softmanx_out[64][16][64][64]);
...
void FlatDataflow(data_t query, data_t key, data_t value, data_t bias);
